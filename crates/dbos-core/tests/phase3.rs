...
async fn receiver_short(ctx: WfCtx, _: ()) -> Result<String, DbosError> {
    ctx.recv::<String>("topic", Duration::from_millis(150)).await
}
...
async fn send_from_outside_recv_in_workflow() {
    ...
    let h = dbos::run_workflow::<(), String>(
        ...
        "receiver",
        (),
        RunOptions { workflow_id: Some("recv-1".into()), ..Default::default() },
    )
    .await
    .unwrap();

    // From outside the workflow, deliver a message.
    dbos::send(&ctx, "recv-1", "hello".to_string(), "topic").await.unwrap();
...
async fn recv_times_out() {
    ...
    let h =
        dbos::run_workflow::<(), String>(&ctx, "receiver_short", (), RunOptions::default()).await.unwrap();
...
async fn send_to_nonexistent_workflow_fails() {
    ...
    ctx.launch().await.unwrap();
    let err = dbos::send(&ctx, "does-not-exist", "x".to_string(), "t").await.unwrap_err();
    assert!(err.is_code(dbos::DbosErrorCode::NonExistentWorkflow), "got {err}");
...
async fn recv_consumes_fifo_order() {
    ...
    let h = dbos::run_workflow::<(), Vec<String>>(
        ...
        "recv_two",
        (),
        RunOptions { workflow_id: Some("fifo-1".into()), ..Default::default() },
    )
    .await
    .unwrap();

    dbos::send(&ctx, "fifo-1", "first".to_string(), "t").await.unwrap();
    dbos::send(&ctx, "fifo-1", "second".to_string(), "t").await.unwrap();
...
async fn recv_result_replayed_on_recovery() {
    ...
    let h = dbos::run_workflow::<(), String>(
        ...
        "receiver",
        (),
        RunOptions { workflow_id: Some("recv-rec".into()), ..Default::default() },
    )
    .await
    .unwrap();
    dbos::send(&ctx, "recv-rec", "once".to_string(), "topic").await.unwrap();
    assert_eq!(h.get_result().await.unwrap(), "once");

    // Recover: recv replays the recorded message (no second consume).
    ctx.system_database().set_workflow_status_pending("recv-rec").await.unwrap();
    let handles = dbos::recover_pending_workflows(&ctx, &["local"]).await.unwrap();
    assert_eq!(handles[0].get_result().await.unwrap(), serde_json::json!("once"));
...
async fn set_event_then_get_from_outside() {
    ...
    let h = dbos::run_workflow::<String, ()>(
        ...
        "setter",
        "complete".to_string(),
        RunOptions { workflow_id: Some("set-1".into()), ..Default::default() },
    )
    .await
    .unwrap();
    ...
    let got: Option<String> =
        dbos::get_event(&ctx, "set-1", "status", Duration::from_secs(1)).await.unwrap();
...
async fn get_event_times_out_to_none() {
    ...
    ctx.launch().await.unwrap();
    let got: Option<String> =
        dbos::get_event(&ctx, "no-such-wf", "k", Duration::from_millis(150)).await.unwrap();
    assert_eq!(got, None);
    ctx.shutdown(Duration::from_secs(2)).await;
}
...
async fn get_event_in_workflow_waits_for_setter() {
    ...
    let sh = dbos::run_workflow::<String, ()>(
        ...
        "setter",
        "ready".to_string(),
        RunOptions { workflow_id: Some("set-2".into()), ..Default::default() },
    )
    .await
    .unwrap();
