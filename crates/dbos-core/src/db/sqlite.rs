//! SQLite [`SystemDatabase`] implementation.
//!
//! SQLite supports the full durability model with no external services, which is
//! what the test suite runs against. SQL is written with `?` placeholders bound
//! left-to-right.
...
/// Single dialect-portable migration shared with the Postgres backend
/// (see `crate::db::MIGRATIONS`).
const MIGRATIONS: &[(i64, &str)] = crate::db::MIGRATIONS;

/// SQLite-backed system database.
pub struct SqliteDb {
    pool: SqlitePool,
}

impl SqliteDb {
    /// Open (creating if missing) a SQLite system database from a URL/path.
    pub async fn connect(url: &str) -> Result<Arc<dyn SystemDatabase>, DbosError> {
        ...
        // In-memory databases are private per connection, so a single connection
        // must serve the whole pool. File DBs get a small pool; the write path is
        // serialized by `busy_timeout` + WAL.
        let max = if is_memory { 1 } else { 5 };
    ...
    /// Build an `SqliteDb` from an existing pool (test/embedding hook).
    pub fn from_pool(pool: SqlitePool) -> Arc<dyn SystemDatabase> {
        Arc::new(SqliteDb { pool })
    }

    async fn begin(&self) -> Result<Transaction<'_, Sqlite>, DbosError> {
        self.pool
            .begin()
            .await
...
impl SystemDatabase for SqliteDb {
    ...
    async fn insert_workflow_status(
        ...
        ON CONFLICT (workflow_uuid) DO UPDATE SET
            recovery_attempts = CASE
                WHEN EXCLUDED.status NOT IN (?, ?) THEN workflow_status.recovery_attempts + ?
                ELSE workflow_status.recovery_attempts
            END,
            updated_at = EXCLUDED.updated_at,
            executor_id = CASE
                WHEN EXCLUDED.status IN (?, ?) THEN workflow_status.executor_id
    ...
    async fn check_child_workflow(
        ...
        let child: Option<Option<String>> = sqlx::query_scalar(
            "SELECT child_workflow_id FROM operation_outputs WHERE workflow_uuid = ? AND function_id = ?",
        )
    ...
    async fn clear_queue_assignment(&self, workflow_id: &str) -> Result<bool, DbosError> {
        let res = sqlx::query(
            "UPDATE workflow_status SET status = ?, started_at_epoch_ms = NULL
    ...
    async fn dequeue_workflows(
        ...
        if let Some((limit, period_secs)) = input.rate_limit {
            ...
            recent = sqlx::query_scalar(
                "SELECT COUNT(*) FROM workflow_status
                 WHERE queue_name = ? AND rate_limited = 1 AND status NOT IN (?, ?) AND started_at_epoch_ms > ?",
            )
        ...
        let ids: Vec<String> = sqlx::query_scalar(
            "SELECT workflow_uuid FROM workflow_status
             WHERE queue_name = ? AND status = ? AND (application_version = ? OR application_version IS NULL)
        ...
        for id in ids {
            ...
            let row: Option<SqliteRow> = sqlx::query(
                "UPDATE workflow_status
                 SET status = ?, application_version = ?, executor_id = ?, started_at_epoch_ms = ?, rate_limited = ?,
    ...
    async fn get_deduplicated_workflow(
        ...
        sqlx::query_scalar(
            "SELECT workflow_uuid FROM workflow_status WHERE queue_name = ? AND deduplication_id = ?",
        )
        .bind(queue_name)
    ...
    async fn transition_delayed_workflows(&self) -> Result<(), DbosError> {
        sqlx::query(
            "UPDATE workflow_status SET status = ?
             WHERE status = ? AND delay_until_epoch_ms IS NOT NULL AND delay_until_epoch_ms <= ?",
        )
        .bind(WorkflowStatusType::Enqueued.as_str())
        .bind(WorkflowStatusType::Delayed.as_str())
        .bind(now_ms())
    ...
    async fn cancel_workflow(&self, workflow_id: &str) -> Result<bool, DbosError> {
        ...
        sqlx::query(
            "UPDATE workflow_status
             SET status = ?, updated_at = ?, completed_at = ?, started_at_epoch_ms = NULL,
        ...
        let exists: Option<i64> =
            sqlx::query_scalar("SELECT 1 FROM workflow_status WHERE workflow_uuid = ?")
                .bind(workflow_id)
                .fetch_optional(&self.pool)
                .await
    ...
    async fn resume_workflow(&self, workflow_id: &str) -> Result<(), DbosError> {
        sqlx::query(
            "UPDATE workflow_status
             SET status = ?, queue_name = ?, recovery_attempts = 0, workflow_deadline_epoch_ms = NULL,
                 deduplication_id = NULL, started_at_epoch_ms = NULL, updated_at = ?, completed_at = NULL
             WHERE workflow_uuid = ? AND status NOT IN (?, ?)",
        )
        .bind(WorkflowStatusType::Enqueued.as_str())
    ...
    async fn get_workflow_children(&self, workflow_id: &str) -> Result<Vec<String>, DbosError> {
        sqlx::query_scalar("SELECT workflow_uuid FROM workflow_status WHERE parent_workflow_id = ?")
            .bind(workflow_id)
            .fetch_all(&self.pool)
            .await
    ...
    async fn get_workflow_steps(&self, workflow_id: &str) -> Result<Vec<StepInfo>, DbosError> {
        let rows = sqlx::query(
            "SELECT function_id, function_name, output, error, child_workflow_id,
             ...
             FROM operation_outputs WHERE workflow_uuid = ? ORDER BY function_id ASC",
        )
        .bind(workflow_id)
        .fetch_all(&self.pool)
        .await
    ...
    async fn fork_workflow(&self, input: ForkInput) -> Result<String, DbosError> {
        ...
        sqlx::query("UPDATE workflow_status SET was_forked_from = 1 WHERE workflow_uuid = ?")
            .bind(&input.original_workflow_id)
            .execute(&mut *tx)
            .await
        ...
        if input.start_step > 0 {
            sqlx::query(
                "INSERT INTO operation_outputs (workflow_uuid, function_id, output, error, function_name, child_workflow_id, started_at_epoch_ms, completed_at_epoch_ms, serialization)
                 SELECT ?, function_id, output, error, function_name, child_workflow_id, started_at_epoch_ms, completed_at_epoch_ms, serialization
                 FROM operation_outputs WHERE workflow_uuid = ? AND function_id < ?",
            )
            .bind(&forked_id)
            .bind(&input.original_workflow_id)
    ...
    async fn has_unconsumed_notification(
        ...
        let exists: Option<i64> = sqlx::query_scalar(
            "SELECT 1 FROM notifications WHERE destination_uuid = ? AND topic = ? AND consumed = 0 LIMIT 1",
        )
        .bind(dest)
        .bind(topic)
        .fetch_optional(&self.pool)
    ...
    async fn consume_oldest_notification(
        ...
        let row: SqliteRow = sqlx::query(
            "UPDATE notifications SET consumed = 1 WHERE message_uuid = ? RETURNING message, serialization",
        )
        .bind(&uuid)
        .fetch_one(&mut *tx)
        .await
    ...
    async fn write_stream(
        ...
        sqlx::query(
            "INSERT INTO streams (workflow_uuid, key, value, \"offset\", function_id, serialization)
             SELECT ?, ?, ?, COALESCE((SELECT MAX(\"offset\") FROM streams WHERE workflow_uuid = ? AND key = ?), -1) + 1, ?, ?",
        )
        .bind(workflow_id)
        .bind(key)
        .bind(value)
    ...
    async fn read_stream(
        ...
        let rows = sqlx::query(
            "SELECT value, \"offset\", serialization FROM streams
             WHERE workflow_uuid = ? AND key = ? AND \"offset\" >= ? ORDER BY \"offset\" ASC",
        )
        .bind(workflow_id)
        .bind(key)
        .bind(from_offset)
        ...
        for r in rows {
            ...
            out.push((value, offset, ser.unwrap_or_default()));
        }
        Ok((out, closed))
    }
    ...
    async fn get_workflow_notifications(
        ...
        let rows = sqlx::query(
            "SELECT topic, message, created_at_epoch_ms, serialization, consumed
             FROM notifications WHERE destination_uuid = ? ORDER BY created_at_epoch_ms ASC",
        )
        .bind(workflow_id)
        .fetch_all(&self.pool)
        .await
    ...
    async fn get_schedule(&self, name: &str) -> Result<Option<ScheduleRow>, DbosError> {
        let row: Option<SqliteRow> = sqlx::query(
            "SELECT schedule_id, schedule_name, workflow_name, workflow_class_name,
                    schedule, status, context, last_fired_at, automatic_backfill,
                    cron_timezone, queue_name
             FROM workflow_schedules WHERE schedule_name = ?",
        )
        .bind(name)
        .fetch_optional(&self.pool)
        .await
    ...
    async fn set_schedule_status(&self, name: &str, status: &str) -> Result<(), DbosError> {
        ...
        sqlx::query(
            "UPDATE workflow_schedules SET status = ?, last_fired_at = NULL WHERE schedule_name = ?",
        )
        .bind(status)
        .bind(name)
        .execute(&self.pool)
...
fn row_to_schedule(row: &SqliteRow) -> ScheduleRow {
    ScheduleRow {
        ...
        automatic_backfill: row.get::<Option<bool>, _>(8).unwrap_or(false),
...
fn row_to_status(
    ...
    Ok(WorkflowStatus {
        ...
        priority: row.get::<Option<i64>, _>(17).unwrap_or(0),
        ...
        was_forked_from: row.get::<Option<bool>, _>(23).unwrap_or(false),
...
mod tests {
    use super::*;
    use crate::db::{NewWorkflowStatus, RecordOperationResultInput};
    ...
    async fn migrate_insert_and_readback() {
        ...
        let st = db.get_workflow_status("wf-1").await.unwrap().unwrap();
        assert_eq!(st.id, "wf-1");
    ...
    #[tokio::test]
    async fn second_insert_keeps_owner_and_increments_on_recovery() {
        let db = fresh().await;
        let mk = |owner: &str, inc: bool| InsertWorkflowStatusInput {
            status: NewWorkflowStatus {
                id: "wf-2".into(),
                status: Some(WorkflowStatusType::Pending),
                name: "wf".into(),
    ...
    #[tokio::test]
    async fn operation_result_record_check_and_conflict() {
        let db = fresh().await;
        db.insert_workflow_status(InsertWorkflowStatusInput {
            status: NewWorkflowStatus {
                id: "wf-3".into(),
                status: Some(WorkflowStatusType::Pending),
                name: "wf".into(),
        ...
        let err = db
            ...
            .await
            .unwrap_err();
        assert!(err.is_code(crate::error::DbosErrorCode::ConflictingId));
    }

    #[tokio::test]
    async fn dead_letter_after_max_retries() {
        let db = fresh().await;
        let mk = || InsertWorkflowStatusInput {
            status: NewWorkflowStatus {
                id: "wf-dlq".into(),
                status: Some(WorkflowStatusType::Pending),
                name: "wf".into(),
        ...
        // attempts: 1 (first), 2, 3 -> 3 > maxRetries+1 (3) is false; 4th -> DLQ
        let mut last = db.insert_workflow_status(mk()).await.unwrap();
        for _ in 0..2 {
            last = db.insert_workflow_status(mk()).await.unwrap();
        }
        assert_eq!(last.attempts, 3);
        let err = db.insert_workflow_status(mk()).await.unwrap_err();
        assert!(err.is_code(crate::error::DbosErrorCode::DeadLetterQueue));
        let st = db.get_workflow_status("wf-dlq").await.unwrap().unwrap();
        assert_eq!(st.status, WorkflowStatusType::MaxRecoveryAttemptsExceeded);
    }
}
